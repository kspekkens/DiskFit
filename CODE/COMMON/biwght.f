      subroutine biwght( val, n, mean, sigma )
c  Copyright (C) 2014, Jerry Sellwood
      implicit none
c estimates the mean and dispersion of a vector of values using the biweight
c  algorithm described by Beers, Flynn, & Gebhardt (1990, AJ, v100, p32)
c  that ignores outliers
c
c calling arguments
      integer n
      real val( n ), mean, sigma
c
...
c ignore outliers
          if( abs( u ) .lt. 1. )then
            num = num + ( val( i ) - med )**2 * ( 1. - u**2 )**4
            den = den + abs( ( 1. - u**2 ) * ( 1. - 5. * u**2 ) )
          end if
        else
